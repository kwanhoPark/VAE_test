...
# config에 따른 모델 구조 보니까 짧은 dat생성하기위해서는 bidi_en + cate_de로 model구성했고
# trio넘어가면 특성다른 midi data를 섞어야되고 길어지니까 hi_en(bi_en) + hi_de(cate_de or split_mul_de)로 구성했네
# max_seq_len값 커지니까 hi 사용하네
# grooVAE랑 perf는 머지?
...
CONFIG_MAP['cat-drums_2bar_small'] = Config(
    ...
    hparams=merge_hparams(
        lstm_models.get_default_hparams(),
        #lstm_model에서 hparams default값 불러오고
        HParams(
            batch_size=512,
            max_seq_len=32,  # 2 bars w/ 16 steps per bar
            z_size=256,
